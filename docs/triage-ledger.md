...
| `e656be5c3c`               | T7180 | **Measured, not taken.** The instrument the earlier deferral asked for was built: a poll storm with a concurrent write load driving computed work, so the contention between computed workers and HTTP consumers exists while it is measured. The fix lowered the default outbox worker concurrency from 8 to 2, and `V2_COMPUTED_OUTBOX_TRIGGER_CONCURRENCY` restores the old value on one binary. Four alternating pairs in hybrid mode: `pollP50Ms` favours the post-fix setting 3 of 4, `pollP95Ms` 2 of 4 and worse by median, throughput 2 of 4. Signs flip, medians move about 5%. Weaker than T7272's A/B on the same harness, which was already called too small. Do not build more harness for this one; it needs an instrument finer than a wall clock.                                                         |
| `bbcecdbfb4`               | T7272 | **Measured, still not taken as a detector.** The `compute-activity-poll` runner exists now, and the coalescer is switched by `COMPUTED_ACTIVITY_READ_CACHE_MS`, so it can be A/B-ed on one binary instead of across two checkouts. Four alternating pairs on `develop`: the coalescer wins 4 of 4 on both throughput (+12.7% median) and `pollP50Ms` (-15%), and the tails overlap. Right sign, plausible size — and about the size of this lab's own run-to-run noise, so one observation cannot tell whether the coalescer is present. `compute-activity/poll-storm-20-viewers-10k` ships as a floor guard and a reusable harness, not as a T7272 regression detector.                                                                                                                                                   |
| `74e773822e`               | T7181 | Skips the reconciliation heal on budgeted ShareDB and activity polls. Now reachable in principle through the same poll storm as T7272 — but the saving is per-poll server work on a path the storm reaches only through HTTP, and T7272, which is the larger of the two savings on that same path, already sits at the noise floor. Worth trying only after the family has an instrument that resolves better than a wall clock.                                                                                                                                                                                                                                                                                                                                                                                           |
...
The other six stand. Two of them — T7272 and T7180 — are now measured rather
than assumed rejections, and both were measured on one binary by flipping the
environment variable that switches the fix, which is a far better instrument
than two checkouts and worth reaching for first whenever a fix has such a
switch. T7075 is the only one of them whose case was written and
