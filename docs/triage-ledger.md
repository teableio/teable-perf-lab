...
| `e656be5c3c`               | T7180 | Worker and query contention: outbox trigger budgets, stage-plan splitting, worker wake-up behaviour. The win is concurrent throughput across pollers and workers, which a single-client run cannot see. The poll-storm harness built for T7272 is the start of the instrument; what this one additionally needs is concurrent _writers_ driving computed work while the pollers run, so the contention it reduces actually exists during the measurement.                                                                                                                                                                                                                                                                                                                                                                  |
...
The other six stand, and T7272 is now a measured rather than an assumed
rejection. T7075 is the only one of them whose case was written and
