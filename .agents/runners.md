...
- **import-base**: `spaceId:"seed-space"`, `sourceBaseNamePrefix`, `tables[]`, `workflows`, `verify{sampleRows,...}`, `threshold{metric:"importBaseStreamMs"|"importBaseTotalReadyMs"}`. Seed caches the source base; execute exports and uploads the `.tea` file as setup, then measures `POST /base/import-stream`. V2 uses stream `done` as primary completion; V1 uses full imported-table readiness as the primary completion point.
