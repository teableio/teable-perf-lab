...
- **conditional-query**: grouped source/host fixture, `sourceRecordCount`, `hostRecordCount`, `groupCount`, `field{kind:"lookup"|"rollup",valueField,filter,expression?,sort?,limit?}`, `verify`, `threshold{metric:"conditionalQueryReadyMs"}`. Use it for fanout, multi-filter, value-type, aggregation, sort/limit, and controlled calculation-volume curves; all configs with the same seed shape share one fixture. Result details expose group matches and retained values per host plus their full-table counts.
