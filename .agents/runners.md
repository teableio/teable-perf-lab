...
| `customer-upsert-computed-flow` | run sequential one-record User + Order upserts through a 40-user/4k-order/400-purchase depth-5 graph | customer write-to-readable flow        |
...
- **customer-upsert-computed-flow**: isolated deterministic Users -> Orders -> Purchases fixture with 10 User profile fields, 10 Order lookups, five Order formula levels, and a Purchase rollup + formula. `scenario` selects update-User/create-Order, update-User/update-Order, or create-User/create-Order. Each write sends exactly one complete record payload; writes are sequential with no fixed sleep. `customerFlowReadyTotalMs` starts at the User request and stops when a target `getRecords` read exposes the complete Order state, while `postOrderResponseReadyMs` separately gates the response-after-write window. Full Users/Orders/Purchases scans and a 50 ms Computed Outbox observer run outside the primary timer; observer data is diagnostic only. `customer-upsert-computed-flow-model.ts` owns deterministic state and affected-row algebra, and the runner uses `record-mutation-lifecycle` for seed/execute/cleanup. Register these cases in the production-hybrid list rather than V2 sync.
