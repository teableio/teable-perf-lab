...
| `conditional-lookup-record-create` | prepare a populated conditional lookup, create a small dirty host batch, verify dirty scope and controls                | lookup-on-record-create regressions    |
...
| `blocked-writer`                   | time an ordinary write issued by a second session while a first session holds a table lock                              | contention a bystander request pays    |
| `compute-activity-poll`            | N viewers polling one table's compute-activity projection at once, against a single-viewer control                      | per-poll cost under many viewers       |
| `circular-link-propagation`        | edit scalar cells on a link-child table in a circular dual-link four-table lookup/formula graph, await host readiness   | child-edit upward computed propagation |
...
| `customer-upsert-computed-flow`    | run one-record Order writes with an optional preceding User write through a 40-user/4k-order/400-purchase depth-5 graph | customer write-to-readable flow        |
...
- **conditional-lookup-record-create**: the same 10k deterministic source/host seed as `conditional-lookup`, plus `mutation{recordCount,sourceStartOffset}`, `verify.dirtySampleRows`, and `threshold{metric:"conditionalLookupRecordCreateReadyMs"}`. Execute creates and fully verifies the lookup as setup, then measures one V2 sync bulk host-record create plus dirty-row readiness and a full seed/control scan. It rides `record-mutation-lifecycle` and restores the shared fixture even inside an isolated execute job because the conditional lookup/rollup siblings reuse the same physical seed later in the shard.
...
- **circular-link-propagation**: four-table incident-shaped fixture (Orders -> SubOrders <=> Purification circular dual links + a tiny Plasmid conditional-lookup source) with `orderRowCount`, `subOrderRowCount`, `purificationRowCount`, `plasmidRowCount`, the three deterministic permutations, `mutation{startOffset?,recordCount}`, `verify{subOrderSampleRows,purificationSampleRows,...}`, and `threshold{metric:"circularPropagationReadyMs"}`. The measured operation edits `expression_mg_l` on the mutation window of Purification rows (the link child) and polls each affected SubOrder by record id until every lookup + formula is readable; the full circular cascade of both wide tables (including Purification's reverse lookup of the changed SubOrders formula) is verified after the primary timer. Field schema, row mappings, and the expected-value algebra live in `circular-link-propagation-workload.ts`; the runner owns Teable I/O and class-C cleanup through `record-mutation-lifecycle`. Register these cases in the production-hybrid list like the other link-computed propagation cases — EXCEPT `lookup/circular-purification-append-400-3k`, which deliberately stays in the V2 sync pool because its hybrid run reproduces a known unfixed propagation loss (T7002/T7018, guarded by teable-e2e-lab `lookup/a-burst-of-new-rows-reaches-every-lookup`).
- **blocked-writer**: the only runner here that times a request it did not send. A seeded single table, then three things in order: the bystander write alone as its own control, the trigger, and the same bystander write released once `pg_locks` shows the trigger holding the mode the case named. `blockedWriterRatio` (bystander over its control) is the reading; `blockedWriterMs` carries the guardrail and `triggerMs` is a diagnostic, because the class of incident this exists for is the one where the causing request looks healthy. `lockObserved` says which kind of run it was — a writer that waited behind the lock, or one released during an operation that never held the table long enough to catch. Not seeing the lock is a result, not an error; a trigger that finishes before the bystander is released is an error, because nothing concurrent was measured. The fixture is never cached (class D): execute lets the product create physical storage, so where a failed run stopped decides what is left behind. See [../docs/blocked-writer-observation-spec.md](../docs/blocked-writer-observation-spec.md).
- **compute-activity-poll**: a 10k table with formula fields, then `GET /api/v2/tables/getComputeActivity` polled first by one viewer and then by `viewers` of them, each keeping one request in flight, all on a worker thread so the poller is not timing the server it shares a process with. `pollP50Ms` is primary and `pollThroughputPerSec` is its companion; the tail percentiles are reported but did not separate at four-pair sample sizes. `pollContentionRatio` compares the storm against the single-viewer control from the same run, which is the only way to read an absolute latency here as anything but hardware. An optional `writeLoad` runs PATCH batches alongside the storm, so the contention between computed workers and HTTP consumers exists while it is measured; that path needs hybrid computed-update mode or the outbox workers never run. Built for T7272 and T7180, both of whose fixes are switched by environment variables and can therefore be A/B-ed on one binary — see the two case descriptions for what that measured and why both remain rejections in the ledger rather than detectors.
...
- **customer-upsert-computed-flow**: isolated deterministic Users -> Orders -> Purchases fixture with 10 User profile fields, 10 Order lookups, five Order formula levels, and a Purchase rollup + formula; the control scenario alone adds a non-computed `sync_marker` User field. `scenario` selects the original update-User/create-Order, update-User/update-Order, and create-User/create-Order flows or the causal controls: Order-create only, first-name-only User update then Order create, control-field User update then Order create, and User-A update then User-B/Purchase-B Order create. Writes are sequential with no fixed sleep; the first-name and control-field requests each send only that one field, while the other User writes retain their complete payload shape. `customerFlowReadyTotalMs` starts at the first execute write and stops when a target `getRecords` read exposes the complete Order state, while `postOrderResponseReadyMs` separately gates the response-after-write window. Full Users/Orders/Purchases scans and a 50 ms Computed Outbox observer run outside the primary timer; observer data is diagnostic only. `customer-upsert-computed-flow-model.ts` owns deterministic state and affected-row algebra, and the runner uses `record-mutation-lifecycle` for seed/execute/cleanup. Register these cases in the production-hybrid list rather than V2 sync.
