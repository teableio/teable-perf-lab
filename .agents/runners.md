...
| `computed-chain-mutation`   | edit one formula or foreign cell in a 40-user/4k-order/400-purchase depth-5 graph       | unchanged-link and schema cascades     |
...
- **computed-chain-mutation**: deterministic Users -> Orders -> Purchases fixture with `userCount`, `orderCount`, `ordersPerUser`, `purchaseGroupSize`, and `targetUserRow`. `mutation` selects one head-formula expression update or one foreign Status/first-name cell update. Formula updates keep their dependency ids unchanged and wait for the full graph inside `fullCascadeReadyTotalMs`; foreign-cell cases stop `firstOrderReadyTotalMs` on the first direct order read, then full-scan all affected and control rows outside the primary timer. `computed-chain-mutation-model.ts` owns the fanout algebra and literal expectations; the runner uses `record-mutation-lifecycle` for seed/execute/cleanup.
