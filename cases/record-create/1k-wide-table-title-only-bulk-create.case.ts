import { definePerfCase } from "../../framework/types";
import { mixedRecordCreate1kBase } from "../record-create.shared";

export default definePerfCase({
  id: "record-create/1k-wide-table-title-only-bulk-create",
  title: "Bulk create 1k title-only rows in a 20-field table",
  runner: "record-create",
  timeoutMs: 600_000,
  config: {
    ...mixedRecordCreate1kBase,
    tableNamePrefix: "perf-record-create-1k-wide-title-only",
    createFieldNames: ["Title"],
    threshold: { metric: "bulkCreate1kMs", maxMs: 6_000 },
  },
});
