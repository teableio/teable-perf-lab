...
Measure deleting one populated number field from a 10,000-row table and isolate
numeric value snapshot/drop cost from other scalar field types.

## Seed Phase

- Create a table containing primary `Title` and number field `Amount`.
...
1. Resolve `Amount`, then start the primary timer.
...
4. Verify `Amount` is absent, `Title` is the only field, and all 10,000
   deterministic titles remain readable.

## Primary Metric

- `deleteFieldMs`: synchronous deletion request latency for one populated
  number field. Seed, id resolution, verification, and cleanup are excluded.
