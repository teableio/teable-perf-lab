...
# field-delete/10k-delete-tags-field

## Goal

Measure deleting one populated multiple-select field from a 10,000-row table
and isolate array-backed option deletion from other field types.

## Seed Phase

- Create a table containing primary `Title` and multiple-select `Tags` with
  four deterministic choices.
- Insert 10,000 populated two-choice arrays in 1,000-row batches.
...
4. Verify `Tags` is absent, `Title` is the only field, and all 10,000
   deterministic titles remain readable.

## Primary Metric

- `deleteFieldMs`: synchronous deletion request latency for one populated
  multiple-select field. Seed, id resolution, verification, and cleanup are excluded.
