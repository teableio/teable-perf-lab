...
Measure deleting one populated long-text field from a 10,000-row table and
isolate multiline payload snapshot/drop cost from other scalar field types.

## Seed Phase

- Create a table containing primary `Title` and long-text `Description`.
...
1. Resolve `Description`, then start the primary timer.
...
4. Verify `Description` is absent, `Title` is the only field, and all 10,000
   deterministic titles remain readable.

## Primary Metric

- `deleteFieldMs`: synchronous deletion request latency for one populated
  long-text field. Seed, id resolution, verification, and cleanup are excluded.
