...
# field-delete/10k-delete-score-field

## Goal

Measure deleting one populated rating field from a 10,000-row table and isolate
bounded numeric option deletion from other scalar field types.

## Seed Phase

- Create a table containing primary `Title` and five-star rating `Score`.
...
4. Verify `Score` is absent, `Title` is the only field, and all 10,000
   deterministic titles remain readable.

## Primary Metric

- `deleteFieldMs`: synchronous deletion request latency for one populated
  rating field. Seed, id resolution, verification, and cleanup are excluded.
