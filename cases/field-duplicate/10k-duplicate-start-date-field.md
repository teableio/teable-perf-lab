...
Measure duplicating one populated date field and its 10,000 serialized values.

## Seed Phase

- Create a table with primary `Title` and date `Start Date` using a fixed
  date-only format and time zone.
...
2. Duplicate it to `Start Date Copy` through the public field endpoint.
