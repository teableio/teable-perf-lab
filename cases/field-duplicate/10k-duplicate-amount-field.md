...
Measure duplicating one populated numeric field and its 10,000 values.

## Seed Phase

- Create a table with primary `Title` and number `Amount`.
- Insert 10,000 deterministic decimal values in 1,000-row batches.
- Full-scan the seed and verify rows 1, 5,000, and 10,000.

## Execute Phase

1. Resolve `Amount`, then start the primary timer.
2. Duplicate it to `Amount Copy` through the public field endpoint.
