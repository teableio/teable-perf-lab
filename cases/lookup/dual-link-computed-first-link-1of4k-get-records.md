...
4,000-row deep computed graph, using the filtered record-list API that matches
the customer's write-then-query pattern. Paired with the direct-record variant,
...
4. After the PATCH response, poll `GET /records` every 100 ms with an exact
   equality filter on that order's deterministic `Title`, requesting at most two
   rows and requiring the single returned id to match the mutated record. Stop
   when all 20 lookups and the complete order formula chain are correct.
...
- The filtered read must return exactly one row, that row must be the mutated
  record id, and all 20 lookup values plus every order formula must be correct.
...
Pair this case with
`lookup/dual-link-computed-first-link-1of4k-get-record`. Every workload input is
held constant except the API used for the primary readiness read, so their
`lookupPropagationMs` values form a direct read-path comparison.
