...
- Verifying a case locally before handing it back: follow the localrun skill.
  It is not published with this repository — ask the team for it. A case is not
  done until it has passed a local v1+v2 run with verified artifacts.
...
- `scripts/perf-run-summary-model.mjs`: Feishu summary projection and the one
  card model — run health, then 相对近期 (same-run quantile), a folded
  对照线上单次 (noisy two-point), and V1; keep webhook/GitHub I/O in
...
- Each seed job uploads its selected dump as a workflow artifact. Execute shard
  N downloads seed dump N into separate V1/V2 Postgres containers, sets the
  target engine, and runs measured operations. Cases that share a physical seed
