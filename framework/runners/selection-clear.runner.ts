...
import { clearSelectionStream } from "@teable/openapi";
import {
  createRecords,
  createTable,
  getFields,
...
const prepareClearFixture = async (
  ...
  if (!viewId) {
    throw new Error(`No grid view found for selection clear table ${table.id}`);
  }
...
export const runSelectionClearCase = async (
  ...
  try {
    prepareMeasurement = await measureAsync("prepare", () =>
      prepareClearFixture(baseId, tableName, config),
    );
