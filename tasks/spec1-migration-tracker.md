...
Tracks which runner kinds have been moved onto the shared lifecycle driver
(the "make it elegant" deepening) and which are still legacy. Migration is
**per runner kind**; a kind's row covers every case that uses it, because
migrating a runner means re-verifying all of its cases (run them v1+v2 and G1
the artifacts — see `tasks/spec1-review-guide.md`).
...
runner onto the driver and ticks its row here. Each migration must keep behavior
identical, proven by the G1 artifact diff over its cases × v1 × v2. The driver
itself becomes truly generic only after a second family migrates (right now it is
record-replay-shaped on purpose). Update the "Migrated" count when a row moves.
